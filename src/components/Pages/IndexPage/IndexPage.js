...
const IndexPageComponent = () => {
    ...
    const LoginComponent = (
        <Col className={style.login_block}>
            ...
            <div className={`${style.function_txt} fs-6 my-2`}>
            ...
            <div className='mb-5'>
                <div className="mb-2 mt-3">
                    ...
                    <div className="my-1">
                        <Button className={`${style.google_btn} fs-6`}>
                            <img src={google} className="me-1" alt="google" style={{ width: '17px' }} />
                            使用Google帳號登入
                        </Button>
                    </div>
                    <div className="my-1">
                        <Button className={`${style.line_btn} fs-6`}>
    ...
    const AboutUsComponent = (
        <Col className={style.about_block}>
            <div className='my-5'>
                ...
                時間，給使用者快速便利的訂餐模式，
                一起讓揪團訂餐變得更輕鬆吧！
                <div className={`${style.byWHO} mt-5`}><span>By </span>開發團隊</div>
            </div>
        </Col>
    );
    ...
    return (
        <div id={style.home}>
            <div className={`${style.main} mx-auto py-2`}>
                <img src={pizza} className={style.pizza} alt="pizza" />
                <img src={rice} className={style.rice} alt="rice" />
                <Row className={style.title} onClick={removeContent}>
                    <Col>
                        <img src={logo} className={`${style.logo} ${content ? style.logo_active : ''}`} alt="logo" />
                    </Col>
                </Row>
                <Row className={`${style.title} ${content ? style.title_active : ''}`} onClick={removeContent}>
                    <Col>What's for Lunch?</Col>
                </Row>
                <Row className={`${style.line} mx-auto ${content ? 'd-none' : ''}`}></Row>

                {/* middle */}
                <Row className={`${style.middle_before} mt-3 mb-4 mx-auto fs-5 ${content ? 'd-none' : ''}`}>
                    <Col>專為團體訂餐設計的線上訂購系統</Col>
                </Row>
                <Row className={`${style.middle_after} mt-3 mb-4 mx-auto fs-5 px-sm-3 px-md-5 px-2 py-2 ${content ? '' : 'd-none'}`}>
                    {content}
                </Row>

                <Row className={`${style.bottom} px-5`}>
                    <Col className="my-1"><Button className={`${style.about_btn} fs-5`} onClick={showAboutUs}>關於我們</Button></Col>
                    <Col className="my-1"><Button className={`${style.login_btn} fs-5`} onClick={showLogin}>登入</Button></Col>
                </Row>
            </div>
            <div id={style.Copyright} className={`${content ? 'd-none' : ''}`}>
